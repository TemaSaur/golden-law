		...
		<div className="images">
			{post.images && typeof post.images == "string" &&
				<div className="image relative overflow-hidden">
					<Image className="max-h-[360px] w-auto mx-auto relative z-10" src={"http://192.168.0.5:3000/" + post.images} width={640} height={360} alt="" />
					<Image className="w-full mx-auto absolute top-0 blur-xl" src={"http://192.168.0.5:3000/" + post.images} width={640} height={360} alt="" />
				</div>
			}
			{post.images && typeof post.images === "array" && post.images.length <= 1 &&
				<div className="image relative overflow-hidden">
					<Image className="max-h-[360px] w-auto mx-auto relative z-10" src={"http://192.168.0.5:3000/" + post.images[0]} width={640} height={360} alt="" />
					<Image className="w-full mx-auto absolute top-0 blur-xl" src={"http://192.168.0.5:3000/" + post.images[0]} width={640} height={360} alt="" />
				</div>
			}{post.images && typeof post.images === "array" && post.images.length > 1 &&
				<div className="flex gap-2 flex-wrap">
					{post.images.map((image, i) =>
						<Image className="max-h-[360px]  w-auto mx-auto relative z-10" src={"http://192.168.0.5:3000/" + image} width={640} height={360} alt="" key={i} />
					)}
				</div>
			}
		</div>
